            ...
            <div class="chat-layout">
                <div class="contacts" id="contacts">
                    ...
                    <?php foreach ($users as $user): ?>
                        <?php if ($user_id != $user->id_user && $user->level == '1') { ?>
                ...
                <div class="chat" id="chat">
                    <div class="p-3" style="background-color: #243e8d; color: white;">
                        <h4 class="ml-1" style="font-weight: bold;" id="selected-contact">.....</h4>
                    </div>
...
function loadMessages(friendId) {
    ...
    $.post('<?= site_url("Chat/getMessages") ?>', { friend_id: friendId }, function (data) {
        try {
...
document.getElementById('send-button').addEventListener('click', function () {
    ...
    const messageText = input.value.trim().replace(/\n/g, '<br>');
    ...
    if (messageText && currentFriendId) {
        ...
        const newMessage = `
            <div class="chat-message user">
                <div class="message">
                    <p>${messageText}</p>
                    <span style="text-align: right; display: block;" class="time-user">${currentTimestamp}</span>
                </div>
            </div>
        `;
        chatMessages.innerHTML += newMessage;
        ...
        const url = '<?= site_url("Chat/sendMessage") ?>';
        $.post(url, {
